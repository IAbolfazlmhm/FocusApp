...
import { setupStepperButtons } from '../../shared/stepper/stepper-utils.js';
...
export function loadSettings() {
  ...
  if (settings) {
    ...
    if (workDur) {
      // Clamp defensively — a value saved before the 1-180 range existed
      // (or corrupted/edited storage) shouldn't be able to hand the
      // stepper an out-of-range or non-numeric value to display.
      const parsed = parseInt(settings.workDuration, 10);
      workDur.value = Number.isNaN(parsed) ? 25 : Math.min(180, Math.max(1, parsed));
    }
