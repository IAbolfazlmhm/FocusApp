...
function attachSmartTooltip(block, html) {
  ...
  // FIX: on desktop, opening the daily-report modal (this block's other
  // click listener, registered right after this call) naturally covers
  // the cursor's position, which fires a real mouseleave and hides the
  // tooltip on its own. Touch has no persistent cursor to "leave" after
  // a tap, so mouseleave never fires there — the tooltip (z-index 99999,
  // above the modal) was staying stuck open, floating over the modal
  // that had just opened underneath it. Hiding it here, on the same
  // click, covers both input types instead of special-casing touch.
  //
  // FIX 2: setting opacity alone still visibly overlapped the modal for
  // a moment even with the above — this tooltip fades out over 150ms
  // (its own transition, above) while the modal fades in over 300ms
  // (see .modal-overlay, modals.css), so for their first ~150ms both
  // were simultaneously on-screen, with the tooltip's higher z-index
  // rendering it right on top of the modal that had just opened
  // underneath. Suppressing the transition for this one change makes it
  // disappear in the same frame the modal starts opening, with no
  // animated overlap window; restoring the transition on the next frame
  // keeps every other hide (mouseleave) fading exactly as before.
